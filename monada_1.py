class User:
    ...
    def get_friend_by_index(self, index):
        try:
...
class Maybe:
    ...
    def __getattr__(self, name):
        # TODO: if hasattr(self.value, name) and getattr(self.value, name) then exception
        field = hasattr(self.value, name) and getattr(self.value, name) or None
        if not callable(field) or not field:
